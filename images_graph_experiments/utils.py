...
class InterferometricModule(nn.Module):
    def __init__(self, K=10,  J=3, N=32):
        super(InterferometricModule, self).__init__()
        self.W_real = nn.Parameter(torch.randn(K,N,N).squeeze()*torch.bernoulli(0.5*torch.ones(K,N,N)))
...
def Fourier_atoms(adj, size_avg):
    ...
    if size_avg == 2 and adj.shape[0] == 50:
        print('NTU')
        FF1, Fc1, Vect01, A1 = Fourrier_Lap_atoms(norm_lap[0:25, 0:25], 1)  # Fourier atoms and conjugate
        FF2, Fc2, Vect02, A2 = Fourrier_Lap_atoms(norm_lap[25:, 25:], 1)  # Fourier atoms and conjugate
    ...
    elif size_avg==2 and adj.shape[0]==30:
        print('SBU')
...
class InterferometricModuleGraphs(nn.Module):
    ...
    def forward(self, x, only_averaging=False, no_averaging=False):
        ...
        y_i = y_i ** 2
        y = y_r + y_i
        y = torch.sqrt(y+1e-6) # for BP, values around 0 can lead to severe round off issues
