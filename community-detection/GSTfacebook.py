...
for itN in range(nSimPoints):
    ...
    if doInterferometric:
        accInterferometric[itN] = [None] * nEdgeFailRealizations
    ...
    for fail in range(nEdgeFailRealizations):
        ...
        if doInterferometric:
            ...
            net = compute_representation(G.W.astype(np.float32),xTrainData.astype(np.float32))
            xTrainData=compute_feature(net,xTrainData.astype(np.float32))
            classifierData.fit(xTrainData, yTrain)
            ...
            #xValidData = xValid.squeeze(1)  # nValid x nFeatures
            xValidData = compute_feature(net, xValid.astype(np.float32))
            yHatValid = classifierData.predict(xValidData)
            # Compute accuracy:
            accValid = np.sum(yValid == yHatValid) / data.nValid
            ...
            xTestData = compute_feature(net, xTest.astype(np.float32))
            yHatTest = classifierData.predict(xTestData)
            # Compute accuracy:
            accTest = np.sum(yTest == yHatTest) / data.nTest
            accInterferometric[itN][fail] = accTest
...
if doFigs:
    ...
    if doInterferometric:
        plt.errorbar(probEdgeFail, meanAccInterferometric, yerr=stdDevAccInterferometric,
                     linewidth=lineWidth,
                     marker=markerShape, markerSize=markerSize)
    plt.xscale('log')
